<template>
  <view class="goods_list_item"
    ...
    <radio
      v-if="ShowRadio"
      class="goods_list_item_radio"
      value="r1"
      :checked="item.goods_checked"
      color="#c00000"
      @click="changeRadioStatus(item)"
    />
    <view class="goods_list_item_image" @click="handleSeeGoodsDetails(item)"
      ><image :src="item.goods_big_logo || defaultPic"></image
    ></view>
    <view class="goods_list_item_context">
      <text
        class="goods_list_item_context_title"
        @click="handleSeeGoodsDetails(item)"
        >{{ item.goods_name }}</text
      >
      <text v-if="ShowNumberBox" class="goods_list_item_context_select"
        >当前选择: X{{ item.goods_counts }}</text
      >
      <view class="goods_list_item_context_bottom">
        <text class="goods_list_item_context_price"
          >￥{{ item.goods_price }}</text
        >
        <uni-number-box
          v-if="ShowNumberBox"
          :min="1"
          :value="item.goods_counts"
          @change="changeNumBoxCount"
        ></uni-number-box>
      </view>
    </view>
  </view>
